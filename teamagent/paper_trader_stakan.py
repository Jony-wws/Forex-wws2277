...
# Market hours: не открывать сделки если рынок закрыт или закрывается
# раньше чем через MARKET_CLOSE_BUFFER_MIN + 1ч.
...
MIN_EXPIRY_H = 1
MAX_EXPIRY_H = 5                       # ⬅ единый диапазон [1..5h] для обеих основных
                                       # стратегий (замечание пользователя 2026-05-01:
                                       # "от 1 часа до 5 часов будет ... на оба стратегии")
...
def _open_new_trades(open_trades: list[dict], snapshot: dict,
    ...
    # Market hours gate: Forex Sun22:00 → Fri22:00 UTC. Сделки в выходные
    # не открываем (никаких живых котировок), а если рынок закроется
    # быстрее чем через 1ч+buffer — тоже стоп.
    if not mh.is_market_open(now_ts):
        log.info("MARKET CLOSED — пропускаю открытие stakan-сделок")
        return 0, signals
    safe_cap_h = mh.max_safe_expiry_hours(now_ts, MARKET_CLOSE_BUFFER_MIN)
    if safe_cap_h < MIN_EXPIRY_H:
        log.info(f"MARKET CLOSING — до закрытия слишком мало (safe_cap={safe_cap_h}h)")
        return 0, signals
    ...
    for pair in config.PAIRS:
        ...
        # market_hours clip — если до закрытия меньше чем нужно, обрезаем
        clipped_h = mh.clip_expiry_hours(int(sig["expiry_hours"]), now_ts,
                                          MARKET_CLOSE_BUFFER_MIN)
        if clipped_h <= 0:
            signals[-1]["skip_reason"] = "market_closing"
            continue
        if clipped_h != int(sig["expiry_hours"]):
            log.info(f"CLIP {pair}: expiry {sig['expiry_hours']}h → {clipped_h}h "
                     f"(до закрытия рынка)")
        expiry_time = now_ts + timedelta(hours=clipped_h)
        sig["expiry_hours"] = clipped_h
        trade = {
