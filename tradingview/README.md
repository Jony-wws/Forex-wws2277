...
| File | Purpose |
|---|---|
| **`eurusd_70wr_strategy.pine`** ⭐ | **Single-file EUR/USD-only** strategy targeting honest **≥70% Win Rate** on M15 with 5h binary expiry.  Acts as **both indicator and strategy in one file** — large BUY/SELL labels on chart + official Strategy Tester results + on-chart stats panel.  Uses Supertrend + Hull MA + multi-TF EMA200 + ADX + VWAP (no RSI/MACD as primary signal).  Hard EUR/USD pair-guard. |
