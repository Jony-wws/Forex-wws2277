# EURNZD — 365-day event-attribution profile

_(period: 365 days, Yahoo 1H bars, real events from FRED+CB+COT+geo archive)_

## Asia (00-06 UTC)
- significant moves (>1.5σ): **18** of 252 cells (7.1%)
- traps (significant move reversed ≥80% within 6h): **4** (22.2% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| cb_rate_decision | 6 | +21.5 | up | 50% | 57% | 17% |

## London (07-12 UTC)
- significant moves (>1.5σ): **27** of 251 cells (10.8%)
- traps (significant move reversed ≥80% within 6h): **12** (44.4% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| eu_cpi | 1 | -86.2 | down | 100% | 92% | 0% |

## Overlap (13-16 UTC)
- significant moves (>1.5σ): **21** of 251 cells (8.4%)
- traps (significant move reversed ≥80% within 6h): **4** (19.0% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| cb_rate_decision | 3 | +26.8 | up | 67% | 47% | 33% |
| cb_press_conference | 3 | +26.8 | up | 67% | 47% | 33% |

## NY (17-21 UTC)
- significant moves (>1.5σ): **22** of 252 cells (8.7%)
- traps (significant move reversed ≥80% within 6h): **14** (63.6% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| cot_release_neutral | 5 | -13.1 | down | 60% | 50% | 0% |
| cot_extreme_short | 1 | -0.3 | up | 100% | n/a | 0% |
