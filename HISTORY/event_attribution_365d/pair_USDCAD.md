# USDCAD — 365-day event-attribution profile

_(period: 365 days, Yahoo 1H bars, real events from FRED+CB+COT+geo archive)_

## Asia (00-06 UTC)
- significant moves (>1.5σ): **23** of 252 cells (9.1%)
- traps (significant move reversed ≥80% within 6h): **13** (56.5% of significant moves)

## London (07-12 UTC)
- significant moves (>1.5σ): **23** of 252 cells (9.1%)
- traps (significant move reversed ≥80% within 6h): **11** (47.8% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| us_claims | 4 | +16.1 | up | 75% | 23% | 25% |
| us_cpi | 2 | -28.3 | down | 100% | 34% | 50% |
| us_core_cpi | 2 | -28.3 | down | 100% | 34% | 50% |
| us_ppi | 2 | -28.3 | down | 100% | 34% | 50% |
| us_nfp | 2 | -34.4 | down | 100% | 36% | 50% |
| us_unrate | 2 | -34.4 | down | 100% | 36% | 50% |
| us_gdp | 1 | +2.9 | up | 100% | 29% | 100% |
| us_pce | 1 | +0.5 | up | 100% | 90% | 100% |

## Overlap (13-16 UTC)
- significant moves (>1.5σ): **19** of 252 cells (7.5%)
- traps (significant move reversed ≥80% within 6h): **1** (5.3% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| us_claims | 5 | +26.7 | up | 60% | 41% | 0% |
| us_nfp | 2 | -54.5 | down | 100% | 100% | 0% |
| us_pce | 2 | -51.5 | down | 100% | 100% | 0% |
| us_core_pce | 2 | -51.5 | down | 100% | 100% | 0% |
| us_unrate | 1 | -71.3 | down | 100% | 100% | 0% |
| us_gdp | 1 | -4.0 | down | 100% | 54% | 0% |
| us_cpi | 1 | +48.9 | up | 100% | 25% | 0% |
| us_core_cpi | 1 | +48.9 | up | 100% | 25% | 0% |

## NY (17-21 UTC)
- significant moves (>1.5σ): **24** of 252 cells (9.5%)
- traps (significant move reversed ≥80% within 6h): **3** (12.5% of significant moves)

**Top event drivers:**

| event_type | n | mean_signed_move_pips | direction (event ccy) | concordance | persistence_24h | trap_rate |
|---|---:|---:|---|---:|---:|---:|
| cot_release_neutral | 8 | -8.3 | up | 75% | 50% | 0% |
| cb_rate_decision | 5 | +2.0 | up | 60% | 69% | 20% |
| cb_press_conference | 5 | +2.0 | up | 60% | 69% | 20% |
| cot_extreme_short | 1 | -48.2 | up | 100% | 0% | 0% |
